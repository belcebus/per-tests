...
def create_per_pattern(
    community: str, year: int, call: str, test_number: str, total_questions: int = 45
) -> ExamPattern:
    """
...
class ParametricExamExtractor:
    ...
    def validate_section_distribution(
        self, exam_section: str, pattern: ExamPattern
    ) -> Dict[int, List[int]]:
        """
        ...
        for i, line in enumerate(lines):
            ...
            if line_clean in category_title_map:
                ...
                current_questions = []
                print(
                    f"📍 Detectada sección: {current_category} - {PER_CATEGORIES[current_category]}"
                )
                continue
        ...
        for category in range(1, 12):  # Categorías 1-11
            ...
            if missing_in_section:
                ...
                validation_warnings.append(
                    f"Categoría {category}: faltan preguntas {sorted(missing_in_section)}"
                )

            if extra_in_section:
                print(f"     ❌ Sobran en sección: {sorted(extra_in_section)}")
                validation_warnings.append(
                    f"Categoría {category}: sobran preguntas {sorted(extra_in_section)}"
                )
        ...
        if missing_global:
            ...
            validation_warnings.append(
                f"Faltan preguntas globalmente: {sorted(missing_global)}"
            )

        if extra_global:
            print(f"🚨 Preguntas extra globalmente: {sorted(extra_global)}")
            validation_warnings.append(
                f"Preguntas extra globalmente: {sorted(extra_global)}"
            )
    ...
    def _is_legitimate_section_title(
        self,
        line: str,
        category_title_map: Dict[str, int],
        ...
        if (
            len(line_words) > len(expected_category_words) + 2
        ):  # Permitir 2 palabras extra
            return False
        ...
        if previous_lines:
            ...
            for i, prev_line in enumerate(previous_lines[-3:]):  # Últimas 3 líneas
                ...
                if (
                    ...
                    or re.match(
                        r"^\d{1,2}\s+", prev_stripped
                    )  # Inicio de nueva pregunta
        ...
        if line_trimmed.lower() == line_clean:
            ...
            if previous_lines and len(previous_lines) > 0:
                ...
                if (
                    ...
                    and not re.match(r"^[a-d]\)", last_line, re.IGNORECASE)
                ):
                    return False
            return True
    ...
    def generate_filename(self, pattern: ExamPattern, base_dir: str = "data") -> str:
        ...
        filename = (
            f"per-{test_code_clean}-{community_clean}-{pattern.year}-{call_clean}.yaml"
        )
        return os.path.join(base_dir, filename)

    def extract_text_from_pdf(
        self, pdf_path: str, start_page: Optional[int] = None
    ) -> str:
        """
        ...
        if start_idx >= total_pages:
            doc.close()
            raise ValueError(
                f"La página de inicio {start_page} excede el total de páginas del documento ({total_pages})"
            )
    ...
    def find_exam_section(
        self, text: str, pattern: ExamPattern, start_page_specified: bool = False
    ) -> Tuple[str, int, int]:
        """
        ...
        if start_page_specified:
            ...
            if subtitle_match:
                print(
                    f"✅ Encontrado subtítulo '{pattern.subtitle}' en posición: {subtitle_match.start()}"
                )
                ...
                print(
                    f"📊 Sección del examen extraída (modo flexible): {len(exam_section)} caracteres"
                )
                ...
                for i, match in enumerate(question_matches):
                    ...
                    if question_num_match:
                        ...
                        if (
                            ...
                            end_pos = match.start()
                            print(
                                f"✅ Final del test detectado: pregunta {question_num} excede límite, terminando en posición: {end_pos}"
                            )
                            break

                exam_section = text[start_pos:end_pos]
                print(
                    f"📊 Sección del examen extraída (modo pregunta directa): {len(exam_section)} caracteres"
                )
        ...
        if not start_match:
            ...
            fallback_pattern = (
                rf"{re.escape(pattern.title)}\s*{re.escape(pattern.subtitle)}"
            )
        ...
        for end_pattern in end_patterns:
            ...
            if match:
                ...
                if candidate_end < end_pos:
                    ...
                    print(
                        f"✅ Final encontrado con patrón: {end_pattern[:30]}... en posición: {candidate_end}"
                    )
                    break
    ...
    def parse_questions_from_section(
        self, exam_section: str, pattern: ExamPattern
    ) -> List[Dict]:
        """
        ...
        while i < len(lines):
            ...
            if line_lower in category_title_map:
                ...
                if self._is_legitimate_section_title(
                    line, category_title_map, previous_lines, next_lines
                ):
                    current_category = category_title_map[line_lower]
                    current_category_name = pattern.categories[current_category]
                    print(
                        f"✅ Título de sección LEGÍTIMO: {current_category} - {current_category_name}"
                    )
                    i += 1
                    continue
                else:
                    print(
                        f"🚫 IGNORADO falso título de sección: '{line}' (parte de opción de respuesta)"
                    )
            ...
            if question_match:
                ...
                if question_num < 1 or question_num > pattern.total_questions:
                    ...
                    if question_num > pattern.total_questions:
                        print(
                            f"🛑 Terminando extracción: encontrada pregunta {question_num} que excede el límite de {pattern.total_questions}"
                        )
                        break
                    i += 1
                    continue
                ...
                is_ripa_reference_only = (
                    ...
                    or re.match(
                        r"^\s*de\s+la\s+Regla\s+\d+\s*$",
                        question_text_start,
                        re.IGNORECASE,
                    )
                    or re.match(
                        r"^\s*Regla\s+\d+\s*$", question_text_start, re.IGNORECASE
                    )
                )
                ...
                while i < len(lines):
                    ...
                    question_pattern_match = re.match(
                        r"^\s*(\d{1,2})\s+([^\d\s].*)", next_line
                    )
                    ...
                    if next_line_lower in category_title_map:
                        ...
                        if self._is_legitimate_section_title(
                            ...
                            print(
                                f"🚫 IGNORADO falso título en pregunta {question_num}: '{next_line}'"
                            )
                            if next_line:
                                question_content += "\n" + next_line
                            i += 1
            ...
            if question_number_only:
                ...
                while i < len(lines):
                    ...
                    next_question_match = re.match(
                        r"^\s*(\d{1,2})\s+([^\d\s].*)", next_line
                    )
                    next_question_number_only = re.match(
                        r"^\s*(\d{1,2})\s*$", next_line
                    )
                    next_line_lower = next_line.lower().rstrip(".")
                    if (
                        next_question_match
                        or next_question_number_only
                        or next_line_lower in category_title_map
                    ):
                        break
                    if next_line:
        ...
        if missing_ids:
            ...
            valid_recovered = [
                q for q in recovered_questions if self._is_valid_question(q)
            ]
    ...
    def _parse_single_question(
        ...
        """
        Parsea el contenido de una pregunta tipo test PER y extrae enunciado y opciones.
        Devuelve un diccionario con id, pregunta, opciones y metadatos.
        """
        # Separar enunciado y opciones (a), b), c), d))
        # Soporta variantes: a), a ), a .
        opciones_regex = r"(?:^|\n)[a-dA-D][\).\-:]\s*"
        partes = re.split(opciones_regex, question_content)
        opciones = re.findall(
            r"([a-dA-D])[\).\-:]\s*([^a-dA-D\).\-:][^\n]*)", question_content
        )

        # El enunciado es la parte antes de la primera opción
        enunciado = partes[0].strip() if partes else question_content.strip()
        opciones_dict = {}
        for idx, (letra, texto) in enumerate(opciones):
            letra = letra.lower()
            opciones_dict[letra] = texto.strip()
        ...
        return {
            ...
            "correct_answer": None,  # Se asigna después
        }
        ...
        for pattern in anulada_patterns:
            ...
            for match in matches:
                ...
                if 1 <= question_num <= 45:
                    answers[question_num] = "ANULADA"
                    print(
                        f"⚠️  Pregunta {question_num} marcada como ANULADA (patrón tradicional)"
                    )
    ...
    def extract_exam(
        self, questions_pdf: str, pattern: ExamPattern, start_page: Optional[int] = None
    ) -> Dict:
        """
        ...
        exam_section, _, _ = self.find_exam_section(
            questions_text, pattern, start_page is not None
        )
        ...
        if not exam_section:
            ...
            if start_page:
                print(
                    f"💡 Verifica que el examen {pattern.subtitle} esté presente desde la página {start_page}"
                )
            else:
                print(
                    "💡 Si la cabecera del examen está en una imagen, prueba con --start-page N"
                )
            return {}
        ...
        print(
            "💡 Consejo: Usa 'python madrid_extract_answers.py --exam-type PER --test-model TEST01' para extraer respuestas"
        )
        ...
        exam_data = {
            "exam_info": {
                ...
                "questions_with_answers": sum(
                    1 for q in questions if q["correct_answer"] is not None
                ),
                # Nuevos metadatos
        ...
        for category_id, category_questions in questions_by_category.items():
            exam_data["categories"][category_id] = {
                "name": pattern.categories[int(category_id)],
    ...
    def _recover_missing_questions(
        ...
        for missing_id in sorted(missing_ids):
            ...
            for i, line in enumerate(lines):
                ...
                if pattern_match:
                    print(
                        f"✓ Posible coincidencia para pregunta {missing_id} en línea {i}: {line[:80]}..."
                    )

                    # Recopilar contenido desde esta línea
                    content_lines = [
                        pattern_match.group(1)
                    ]  # Usar solo el texto después del número
                    j = i + 1
                    collected_chars = len(pattern_match.group(1))

                    while (
                        j < len(lines) and collected_chars < 1000
                    ):  # Límite de seguridad
                        ...
                        next_question_match = re.match(
                            r"^\s*(\d{1,2})\s+([^\d\s].*)", current_line
                        )
                        if next_question_match:
                            next_num = int(next_question_match.group(1))
                            if (
                                1 <= next_num <= pattern.total_questions
                                and next_num != missing_id
                            ):
                                break
                    ...
                    if content_lines:
                        ...
                        print(
                            f"✅ Recuperada pregunta {missing_id} en categoría {category_name}"
                        )

                        question_data = self._parse_single_question(
                            missing_id, content, category, category_name
                        )
                        if question_data:
    ...
    def _is_valid_question(self, question_data: Dict) -> bool:
        ...
        if len(question_text) < 10:
            print(
                f"❌ Pregunta {question_data.get('id')} rechazada: texto muy corto ('{question_text}')"
            )
            return False
        ...
        if question_text.lower() in invalid_fragments:
            print(
                f"❌ Pregunta {question_data.get('id')} rechazada: fragmento inválido ('{question_text}')"
            )
            return False
        ...
        if not expected_options.issubset(set(options.keys())):
            ...
            print(
                f"❌ Pregunta {question_data.get('id')} rechazada: faltan opciones {missing_options}"
            )
            return False
        ...
        for letter, option_text in options.items():
            if len(option_text.strip()) < 3:
                print(
                    f"⚠️  Pregunta {question_data.get('id')} opción {letter} muy corta: '{option_text.strip()}' (revisar manualmente)"
                )
        ...
        has_question_structure = (
            "?" in question_text
            or ":" in question_text  # Muchas preguntas terminan con ":"
            or question_text.lower().startswith(
                (
                    "qué",
                    "cuál",
                    "cómo",
            ...
            or len(question_text.split()) >= 4  # Relajado: al menos 4 palabras
            or question_text.strip().endswith("...")  # O termina en puntos suspensivos
        )

        if not has_question_structure:
            print(
                f"❌ Pregunta {question_data.get('id')} rechazada: no parece una pregunta válida ('{question_text[:50]}...')"
            )
            return False
    ...
    def _remove_duplicate_ids(self, questions: List[Dict]) -> List[Dict]:
        ...
        for question_id, question_list in questions_by_id.items():
            if len(question_list) == 1:
                ...
                print(
                    f"⚠️  Detectados {len(question_list)} duplicados para pregunta {question_id}"
                )
                ...
                for i, q in enumerate(question_list):
                    ...
                    print(
                        f"   {marker}: '{q['question'][:50]}...' (categoría {q['category']})"
                    )
    ...
    def _choose_best_question(self, question_candidates: List[Dict]) -> Dict:
        ...
        valid_candidates = [
            q for q in question_candidates if self._is_valid_question(q)
        ]
        ...
        def quality_score(question):
            ...
            if any(
                ...
                for starter in ["qué", "cuál", "cómo", "indique", "señale"]
            ):
                score += 15
...
El archivo de salida seguirá el formato: per-{test}-{comunidad}-{año}-{convocatoria}.yaml
    ...
    parser.add_argument(
        "--input-file", required=True, help="Archivo PDF con las preguntas del examen"
    )
    parser.add_argument(
        "--test-code",
        required=True,
    ...
    if match:
        ...
        if args.verbose:
            print(
                f"⚠️  No se pudo detectar año/convocatoria del nombre del archivo, usando valores por defecto: {year} {call}"
            )
    ...
    try:
        ...
        print(
            f"   ✅ Preguntas extraídas: {exam_info.get('total_questions', 0)}/{exam_info.get('expected_questions', 0)}"
        )
        print(f"   📚 Categorías encontradas: {exam_info.get('categories', 0)}")
        print(
            f"   📝 Preguntas con respuestas: {exam_info.get('questions_with_answers', 0)}"
        )
        ...
        print(
            f"💡 Verifica que el PDF contenga el examen especificado ({pattern.subtitle})"
        )
        return 1
