...
class Settings(BaseSettings):
    ...
    Utiliza variables de entorno con prefijo PER_ o valores por defecto si no están definidas.
    """
    ...
    data_dir: str = Field(
        default="data", alias="PER_DATA_DIR", description="Directorio base de datos"
    )
    ...
    host: str = Field(
        default="0.0.0.0", alias="PER_HOST", description="Host del servidor"
    )

    port: int = Field(default=8002, alias="PER_PORT", description="Puerto del servidor")

    reload: bool = Field(
        default=True, alias="PER_RELOAD", description="Recarga automática en desarrollo"
    )

    log_level: str = Field(
        default="info", alias="PER_LOG_LEVEL", description="Nivel de logging"
    )
    ...
    api_title: str = Field(
        default="PER Tests API", alias="PER_API_TITLE", description="Título de la API"
    )
    ...
    api_version: str = Field(
        default="1.0.0", alias="PER_API_VERSION", description="Versión de la API"
    )
    ...
    debug: bool = Field(default=False, alias="PER_DEBUG", description="Modo debug")
