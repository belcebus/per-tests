...
class TestMainApplication:
    ...
    def _get_all_route_paths(self):
        """Obtiene todas las rutas configuradas en la aplicación (incluyendo routers incluidos)."""
        paths = []
    ...
    def test_app_has_routes(self):
        ...
        routes = self._get_all_route_paths()
...
class TestLifespanManager:
    ...
    async def test_lifespan_startup_directory_exists(
        self, mock_print, mock_question_loader
    ):
        ...
        mock_exams_dir.glob.return_value = [
            MagicMock(name="test1.yaml"),
            MagicMock(name="test2.yaml"),
        ]
        ...
        async with lifespan(test_app):
            ...
            assert any(
                "🚀 Iniciando aplicación PER Tests..." in msg
    ...
    @patch("app.main.question_loader")
    @patch("builtins.print")
    async def test_lifespan_startup_directory_not_exists(
        self, mock_print, mock_question_loader
    ):
...
class TestErrorHandling:
    ...
    def test_application_with_question_loader_error(self, mock_question_loader):
        ...
        mock_question_loader.load_all_questions.side_effect = Exception(
            "Error de prueba"
        )
